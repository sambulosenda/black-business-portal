...
export const STRIPE_FEE_FIXED = 0.30 // Stripe's fixed fee in dollars
...
export function calculateFees(amount: number) {
  ...
  // Stripe fee (2.9% + $0.30)
  const stripeFee = Math.round(amountInCents * (STRIPE_FEE_PERCENTAGE / 100) + (STRIPE_FEE_FIXED * 100))
