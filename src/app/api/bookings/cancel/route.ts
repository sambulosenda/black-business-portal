  ...
  try {
    ...
    if (isCustomer && !isBusinessOwner) {
      ...
      if (hoursBeforeAppointment < 24) {
        return NextResponse.json(
          {
            error:
              'Cancellations must be made at least 24 hours before the appointment. Contact the business directly for assistance.',
          },
          { status: 400 }
        )
      }
    ...
    return NextResponse.json({
      ...
      booking: {
        ...
        message:
          booking.paymentStatus === 'SUCCEEDED' && isCustomer
